...
// count the number of parts in a circuit board

// javac PartsCount.java
// java PartsCount < someboard.kicad_pcb
...
public class PartsCount {
            ...
            throws Exception
    {
        ...
        for (Value topchilds : topvalue.children) {
            ...
            // get value from (fp_text value <value> ...)
            ...
            for (Value pvv : pv.children) {
                ...
                if (! ((SymbolValue) pvvv.children.get (1)).symbol.equals ("value")) continue;
                compvalue = ((SymbolValue) pvvv.children.get (2)).symbol;
                break;
            }
            if (compvalue != null) {
                String key = comptype + " " + compvalue;
                Integer val = compcounts.get (key);
                if (val == null) val = 0;
                compcounts.put (key, val + 1);
            }
        }

        // print resultant counts
        for (String key : compcounts.keySet ()) {
            System.out.println (key + " = " + compcounts.get (key));
        }
    }
