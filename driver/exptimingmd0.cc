...
// measures clock frequency of MQ1->MD0 flipflop going through bi-dir level converters
...
int main (int argc, char **argv)
{
    ...
    while (true) {
        ...
        for (int i = 0; i < nloops; i ++) {
            ...
            gpiolib->writegpio (false, 0);
            waithird ();
            uint32_t rcvd = gpiolib->readgpio () & G_DATA0;
            bad += (sent / G_DATA1) ^ (rcvd / G_DATA0);
        }
